...
from faux.runtime.signals import SignalFormat

def fmtlabel(y: Union[str, int, Tensor]):
   if isinstance(y, str):
      raise NotImplementedError('str not yet handled')
   elif isinstance(y, Tensor):
      if y.requires_grad:
         y = y.detach()
      assert y.dim() == 0, f'y must be a scalar, got {y.shape}'
      ...
      raise NotImplementedError(f'unhandled type {type(y)}')
...
class Backtester:
   ...
   def loop(self):
      ...
      if self._batched_samples:
         ...
         for i in range(min(len(bT), len(bX), len(by))-1):
            ...
            yp = ypred[i]
            yt, yp = fmtlabel(yt), fmtlabel(yp)
            
            #* maintain measure of effective accuracy
            if yt == yp:
               self.right += 1
            else:
               self.wrong += 1
   ...
   def summarize(self, plot=False, savefig=None, showfig=True, block=True):
