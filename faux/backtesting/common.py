...
from mkt.data import load_frame
...
def samples_for(symbol:Union[str, DataFrame], 
                analyze:Callable[[DataFrame], DataFrame]=None, 
                xcols=None, 
                xcols_not=[], 
                x_timesteps=1, 
                y_type='pl_binary', 
   ...
   elif y_type == 'pl_binary':
      df[y_column] = (df['close'].shift(-y_lookahead) > df['close']).astype('int')
   else:
      raise NotImplemented(f'Handling of y_type="{y_type}" :C')
   
   df = analyze(df)
   # if y_column != '$LABEL':
   #    xcols_not = set(xcols_not) | {y_column}
      
   df = df.fillna(method='bfill').dropna()
   
   if xcols is None:
      xcols = list(set(df.columns) - set(xcols_not) - {y_column})
   ...
   ycol = cols.index(y_column)
   
   data:np.ndarray = df.to_numpy()
   
   if scaling:
      #* scale the data when requested
      scaler_type = scaler
      scaler = scaler_type()
      
      scaled_x = scaler.fit_transform(data[:, xcols])
      data[:, xcols] = scaled_x
