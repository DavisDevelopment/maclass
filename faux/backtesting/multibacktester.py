...
from fn import F, _
from mkt.tools import dpluck, nor, unzip, nn, isiterable, Struct, safe_div
from typing import *
...
# import pandas as pd
from mkt.coretools import list_stonks, load_frame
...
from mkt.tools import maxby, minby, flatten_dict
...
class PolySymBacktester:
   ...
   def run(self, **kwargs):
      ...
      print('DONE')
      summary_kw = valfilter(nn, dpluck(kwargs, 'plot', 'showfig', 'block', 'savefig'))
      summary = self.summary(**summary_kw)
      
      return summary
   
   def summary(self, plot=False, showfig=True, block=True, savefig=None, drop_syms=True)->DataFrame:
...
import itertools
from mkt.tools import isiterable
...
def backtest(symbols:List[str], params, transforms, model=None, dir='./stonks', date_begin=None, date_end=None, **kwargs):
